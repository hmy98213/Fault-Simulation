...
def gen_matrix_cir(matrix):
    nqubit = int(np.log2(matrix.shape[0]))
    qc = QuantumCircuit(nqubit)
    qc.unitary(matrix, range(nqubit))
    trans_qc = transpile(qc, basis_gates=['cx', 'u3'])
    return trans_qc

def my_QPE(num_evaluation_qubits, unitary):
    dim_u = int(np.log2(unitary.shape[0]))
    qpe = QuantumCircuit(num_evaluation_qubits+dim_u)
    for qubit in range(num_evaluation_qubits):
        qpe.h(qubit)
    mat = unitary
    for counting_qubit in range(num_evaluation_qubits):
        qc = QuantumCircuit(dim_u)
        qc.unitary(mat, range(dim_u))
        cqc = transpile(qc.control(), basis_gates=['cx', 'u3'])
        #print(cqc.size())
        #print(cqc)
        qpe = qpe.compose(cqc, [counting_qubit]+list(range(num_evaluation_qubits, num_evaluation_qubits + dim_u)))
        mat = mat.dot(mat)
    qpe = qpe.compose(QFT(num_evaluation_qubits), range(num_evaluation_qubits))
    print("qpe circuit")
    #print(qpe)
    print(qpe.size())
    trans_qc = transpile(qpe, basis_gates=['cx', 'u3'])
    #print(trans_qc)
    #print(trans_qc.size())
    return qpe
...
class HHL(LinearSolver):
    ...
    def my_construct_circuit(
        ...
        matrix_circuit = gen_matrix_cir(matrix)
        ...
        # QPE
        phase_estimation = my_QPE(nl, matrix)
        if na > 0:
