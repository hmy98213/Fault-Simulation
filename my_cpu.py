...
def dag_to_error2(dag, qubits0, qubits1, error_num, error_pos):
    ...
    cnt = 0
    for node in dag.topological_op_nodes():
        if node.name == 'cx' or node.name == 'cz' or node.name == 'cp':
            gate = CNOT
        elif node.name == 'ccx':
            gate = CCX
        elif node.name == "swap":
...
#apply an error circuit with double qubit
def error_cir_apply2(cir, qubits0, qubits1, error_num, error_pos):
    pos = 0
    cnt = 0
    ...
    for gate in cir.data:
        if gate[0].name == 'cx' or gate[0].name == 'cz' or gate[0].name == 'cp':
            mat = CNOT
            ...
            print(gate)
            mat = np.array(gate[0].to_matrix(), dtype=complex)
            print(mat)
            # print(mat.type())
            # if mat.size() == 16:
            #     mat = gate.reshape(2, 2, 2, 2)
...
def one_shot2(cir, ps1, ps20, ps21, error_num, error_pos):
    ...
    with tn.NodeCollection(all_nodes):
        ...
        # cir_to_error2(cir, qubits0, qubits1, error_num, error_pos)
...
#calculate fidelity with only one input
def cal_fidelity_in(cir, ps1, error_num, error_pos):
    all_nodes = []
    with tn.NodeCollection(all_nodes):
        right_vec0, right_vec1, left_vec0, left_vec1 = [arr_to_tnvec(ps1)]*4
        start_gates0 = [
            tn.Node(np.eye(2, dtype=complex)) for _ in range(cir.num_qubits)
        ]
        start_gates1 = [
        ...
        start_wires0 = [node[0] for node in start_gates0]
        start_wires1 = [node[0] for node in start_gates1]
        error_cir_apply2(cir, qubits0, qubits1, error_num, error_pos)
        # cir_to_error2(cir, qubits0, qubits1, error_num, error_pos)
...
def file_test(path, file_name, output):
    f = open(output, 'a')
    f.write("\n")
    ...
    k0 = np.array([1.0, 0], dtype=complex)
    k1 = np.array([0, 1.0], dtype=complex)
...
def folder_test(path, output_file):
    files = os.listdir(path)
    for f in files:
        if f.startswith('gf'):
            continue
        try:
            file_test(path, f, output_file)
        except:
            raise
            pass
        gc.collect()
...
def auto_run(path, file_name, output):
    ...
    if file_name[0] == 'q':
        ...
        ps2b = [np.array([1.0, 0], dtype = complex) for i in range(nqubits)]
