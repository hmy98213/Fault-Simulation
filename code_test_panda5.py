...
def cir_test(cir, output_target, input, output):
    ...
    try:
        ...
        with time_limit(3600):
            ...
            result, assign_time, cal_time = test_cir_bdd.noise_measure(output, nqubits, noise_num)
            #run_time = time.time() - t_start
            bdd_num = len(test_cir_bdd.Fa)
            with open(output_target, "a") as f:
                f.write(str(bdd_num) + "\t" + str(build_time) + "\t" + str(assign_time) + "\t" + str(cal_time) + "\t"+ str(result) + "\n")
