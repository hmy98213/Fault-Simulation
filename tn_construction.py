...
class QCTN:
    ...
    def __init__(self, file, path):
        ...
        ket_zero = [
            np.array([1.0, 0], dtype=complex) for _ in range(self.cir.num_qubits)
        ]
        self.ps1 = ket_zero.copy()
        self.ps2 = ket_zero.copy()
        self.all_nodes = []
    ...
    def gen_idx(self, N, n):
        ...
        for e in self.gen_idx(N, n - 1):
            ...
            for i in range(N):
                if first_nonzero == False and e[i] == 0:
                    continue
                if first_nonzero == False and e[i] != 0:
                    first_nonzero = True
                if e[i] == 0:
    ...
    def error_cir_apply(self, qubits0, qubits1):
        ...
        for gate in self.cir.data:
            ...
            if cnt < self.error_num and pos == self.error_pos[cnt]:
                ...
                noise_gateH = np.reshape(
                    np.array(np.matrix(np.reshape(self.noise_gate, (4, 4))).H),
                    (2, 2, 2, 2),
                )
    ...
    def construct_tn_no_error_vec(self, ps1):
        with tn.NodeCollection(self.all_nodes):
            left_vec = arr_to_tnvec(ps1)
            start_gates = [
                tn.Node(np.eye(2, dtype=complex)) for _ in range(self.cir.num_qubits)
    ...
    def construct_enumurate_tn_dm(self, ps1, op_list):
        with tn.NodeCollection(self.all_nodes):
            left_vec = arr_to_tnvec(ps1)
            start_gates = [
                tn.Node(np.eye(2, dtype=complex)) for _ in range(self.cir.num_qubits)
    ...
    def simu_vec(self, output, ps1=[]):
        self.all_nodes = []
        if len(ps1) > 0:
            self.set_io(ps1, [])
        try:
            qubits = self.construct_tn_no_error_vec(self.ps1)
            t_start = time.time()
            # For test
    ...
    def enumurate_io(
        self, output, error_num=0, random_pos=True, ps1=[], ps2=[], enum=1
    ):
    ...
    def enumurate_dm(self, output, error_num=0, random_pos=True, ps1=[], enum=1):
        self.set_error(error_num, random_pos)
        self.print_info(output)
        if len(ps1) > 0:
            self.set_io(ps1, [])
        try:
            ...
            for e in range(enum + 1):
                for id in self.gen_idx(error_num, e):
                    ...
                    op_list = self.idx_to_op(id)
                    self.all_nodes = []
                    qubits0 = self.construct_enumurate_tn_dm(self.ps1, op_list[0])
                    tmp0 = tn.contractors.auto(
                        self.all_nodes, output_edge_order=qubits0
                    ).tensor
                    self.all_nodes = []
                    qubits1 = self.construct_enumurate_tn_dm(self.ps1, op_list[2])
                    tmp1 = tn.contractors.auto(
                        self.all_nodes, output_edge_order=qubits1
                    ).tensor.conjugate()
                    # calculate the tensor product of tmp0 and tmp1
                    tmp = np.kron(tmp0, tmp1).conjugate()
                    for i in op_list[1]:
                        tmp *= i
                    res += tmp
            # res = np.abs(res)
    ...
    def enumurate_fidility_test(
        self, output, error_num=0, random_pos=True, ps1=[], enum=1
    ):
        self.set_error(error_num, random_pos)
        self.print_info(output)
        if len(ps1) > 0:
    ...
    def simu_dm(self, output, error_num=0, random_pos=True, ps1=[], ps2=[]):
        self.set_error(error_num, random_pos)
        self.print_info(output)
        if len(ps1) > 0:
            self.set_io(ps1, [])
        try:
            qubits = self.construct_tn_dm(self.ps1)
            t_start = time.time()
            result = tn.contractors.auto(
                self.all_nodes, output_edge_order=qubits
            ).tensor.conjugate()
